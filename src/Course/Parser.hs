...
list1 ::
  ...
  -> Parser (List a)
list1 p =
  bindParser (\ a -> mapParser (\ t -> a :. t) (list p)) p
...
satisfy p =
  bindParser (\ a ->
               if p a then
                 ...
                 unexpectedCharParser a) character
...
sequenceParser ::
  ...
  -> Parser (List a)
sequenceParser =
  let f p p' = bindParser (\h -> mapParser (\t -> h :. t) p') p in
...
firstNameParser ::
  Parser Chars
firstNameParser =
  bindParser (\ c -> mapParser (\l -> c :. l) (list lower)) upper
        ...
        (\sn ->
          ...
          (bindParser
           (\sm ->
             ...
             (bindParser
              (\fone ->
                valueParser (Person old fn sn sm fone))
              phoneParser))
           smokerParser))
        surnameParser))
     firstNameParser))
